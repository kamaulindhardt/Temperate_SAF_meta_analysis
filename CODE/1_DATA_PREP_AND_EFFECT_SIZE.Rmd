...
```{r}
##############
# SMART SELECTION OF IMPUTATION METHOD - MERGING IMPUTED _SE
############################################################################


# Step 1: Create a combined dataset with both PMM and Upper Quartile imputations
combined_imputed_data <- imputed_datasets$pmm %>%
  select(id_article, id_obs, silvo_se_pmm = silvo_se, control_se_pmm = control_se) %>%
  full_join(
    imputed_datasets$upper_quartile %>%
      select(id_article, id_obs, silvo_se_uq = silvo_se, control_se_uq = control_se),
    by = c("id_article", "id_obs")
  )

# Step 2: Apply conditional logic to select the imputation method
smart_imputed_data <- combined_imputed_data %>%
  mutate(
    silvo_se = ifelse(silvo_se_pmm <= silvo_se_uq, silvo_se_pmm, silvo_se_uq),
    control_se = ifelse(control_se_pmm <= control_se_uq, control_se_pmm, control_se_uq),
    imputation_method = case_when(
      silvo_se_pmm <= silvo_se_uq & control_se_pmm <= control_se_uq ~ "PMM",
      TRUE ~ "Upper Quartile"
    )
  ) %>%
  select(id_article, id_obs, silvo_se, control_se, imputation_method)

# Step 3: Merge the smart-imputed data back into the original dataset
merged_data <- database_clean_sd_df %>%
  ...
  relocate(
    ...
    silvo_se_original, silvo_se_imputed, silvo_sd_original, silvo_sd_from_se, silvo_sd_merged, 
    control_se_original, control_se_imputed, control_sd_original, control_sd_from_se, control_sd_merged, 
    # Imputation info
     control_se_imputed, imputation_method
  )

# Step 4: Recalculate _sd_from_se for imputed data after merging
merged_data <- merged_data %>%
  mutate(
    # Recalculate _sd_from_se for silvo group
    silvo_sd_from_se = ifelse(
      is.na(silvo_sd_original) & !is.na(silvo_se_imputed),  # Only recalculate for missing original SD
      silvo_se_imputed * sqrt(silvo_n),
      silvo_sd_from_se  # Keep existing values
    ),
    # Recalculate _sd_from_se for control group
    control_sd_from_se = ifelse(
      is.na(control_sd_original) & !is.na(control_se_imputed),  # Only recalculate for missing original SD
      control_se_imputed * sqrt(control_n),
      control_sd_from_se  # Keep existing values
    )
  )
...
# Additional Context:
# - This logic ensures that PMM is used when it provides lower _se values than Upper Quartile,
#   maintaining a balance between minimizing distortion and preserving statistical properties.
# - The `imputation_method` column indicates which method was chosen for each observation.
# - Recalculation of _sd_from_se ensures compatibility across all rows, regardless of imputation method.
```
...
original_data <- database_clean_sd_df %>%
  select(id_article, id_obs, silvo_se, control_se, silvo_sd_from_se, control_sd_from_se) %>%
...
imputation_analysis_data <- merged_data %>%
  select(imputation_method, silvo_se_imputed, control_se_imputed, silvo_sd_from_se, control_sd_from_se) %>%
  pivot_longer(
    cols = c(silvo_se_imputed, 
             control_se_imputed, 
             silvo_sd_from_se, 
             control_sd_from_se),
    names_to = "variable",
...
# Create a boxplot for _se_imputed and _sd_from_se by imputation_method
boxplot <- ggplot(imputation_analysis_data, aes(x = imputation_method, y = value, fill = variable)) +
  ...
  facet_wrap(~variable, scales = "free_y") +  # Separate plots for different variables
  labs(
    title = "Distribution of _se_imputed and _sd_from_se by Imputation Method",
    x = "Imputation Method",
    y = "Value",
    fill = "Variable"
  ) +
...
# Create a density plot for a smoother view of distributions
density_plot <- ggplot(imputation_analysis_data, aes(x = value, fill = imputation_method)) +
  geom_density(alpha = 0.6) +
  facet_wrap(~variable, scales = "free") +  # Separate plots for different variables
  labs(
    title = "Density of _se_imputed and _sd_from_se by Imputation Method",
    x = "Value",
    y = "Density",
    fill = "Imputation Method"
  ) +
  scale_x_continuous(trans = "pseudo_log") +  # Apply pseudo-log scale to x-axis
  theme_minimal()
...
Again, checking the imputed control_se and silvo_se density distribution (imputation = PMM + upper quartile)
```{r}
...
non_imp_dataset <- database_clean_sd |> 
  ...
  relocate(
    ...
    silvo_mean, control_mean, silvo_n, control_n,
    # Quantitative meta-analysis variables - variance info
    silvo_se_original, silvo_sd_original, silvo_sd_from_se, silvo_sd_merged, 
    control_se_original, control_sd_original, control_sd_from_se, control_sd_merged 
    # Imputation info
    # control_se_imputed, imputation_method
  )
...
imp_pmm_best_categorical <- imp_pmm_best %>%
...
# Check if datasets have categorical columns and harmonize column names
if (ncol(imp_pmm_best_categorical) > 0 && ncol(non_imp_dataset_categorical) > 0) {
  common_columns <- intersect(names(imp_pmm_best_categorical), names(non_imp_dataset_categorical))
  imp_pmm_best_categorical <- imp_pmm_best_categorical %>% select(all_of(common_columns))
  ...
  if (ncol(imp_pmm_best_categorical) > 0 && ncol(non_imp_dataset_categorical) > 0) {
    ...
    cat_plot <- inspect_cat(imp_pmm_best_categorical, non_imp_dataset_categorical) %>% show_plot()
    print(cat_plot)
  } else {
...
imp_pmm_best_numeric <- imp_pmm_best %>% select(where(is.numeric)) %>% filter_valid_columns()
non_imp_dataset_numeric <- non_imp_dataset %>% select(where(is.numeric)) %>% filter_valid_columns()

# Check if datasets have numeric columns
if (ncol(imp_pmm_best_numeric) > 0 && ncol(non_imp_dataset_numeric) > 0) {
  ...
  # Combine datasets and reshape for ggplot
...
# -----------------------------
if (ncol(imp_pmm_best) > 0 && ncol(non_imp_dataset) > 0) {
  # Correlation inspection
  cor_plot <- inspect_cor(imp_pmm_best, non_imp_dataset) %>% show_plot()
  print(cor_plot)

  # Missingness comparison
  na_plot <- inspect_na(imp_pmm_best, non_imp_dataset) %>% show_plot()
  print(na_plot)
} else {
...
unique_high_variance_obs <- high_variance_data %>%
  distinct(id_article, id_obs, response_variable, measured_metrics, yi, vi,
           ...
           silvo_n, control_n,
           silvo_se_original, control_se_original,
           silvo_se_imputed, control_se_imputed,
           silvo_sd_from_imputed_se, control_sd_from_imputed_se,
           silvo_sd_original, control_sd_original,
...
The Kolmogorov-Smirnov (KS) test was conducted to compare the distributions of effect sizes (ROM) between the non-imputed and imputed datasets. The test statistic (\(D = 0.0332\)) and its corresponding p-value (\(p = 0.6425\)) suggest no significant difference between the two distributions. This indicates that the imputed and non-imputed datasets share similar underlying distributions, and the null hypothesis of no difference cannot be rejected. While the warning about approximate p-values due to ties in the data introduces a potential limitation, the results strongly support that the imputation process preserved the original distributional properties of effect sizes.

However, the metrics for heterogeneity and between-study variance reveal notable differences between the two datasets. The non-imputed dataset demonstrated low heterogeneity (\(I^2 = 19.85\%\)) and minimal between-study variance (\(\tau^2 = 2.78 \times 10^{-6}\)), indicating limited variability among studies. In contrast, the imputed dataset exhibited extremely high heterogeneity (\(I^2 = 99.98\%\)) and a markedly higher between-study variance (\(\tau^2 = 0.0169\)). These differences suggest that the imputation process introduced substantial variability into the dataset, altering its structure.

Warnings regarding the extreme ratio of the largest to smallest sampling variances highlight potential instability in the meta-analytic models, particularly for the imputed dataset. This instability is a common challenge in meta-analyses with imputed data and necessitates cautious interpretation of results when variability in sampling variances is pronounced.

The KS test results confirm that the imputation process maintained the overall distribution of effect sizes. However, the sharp increase in heterogeneity and between-study variance in the imputed dataset suggests that imputation has introduced considerable variability, which could influence the reliability of meta-analytic conclusions. Transparency in reporting these differences and conducting sensitivity analyses to evaluate the impact of imputation are crucial to ensuring the robustness and validity of the findings.

While the imputation successfully preserved the original distributional characteristics of effect sizes, the significant increase in variability underscores the need for careful interpretation and additional analyses to mitigate the potential effects of imputation on the overall results.
