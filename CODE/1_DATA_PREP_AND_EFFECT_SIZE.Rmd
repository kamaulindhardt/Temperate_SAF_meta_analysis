...
Perform imputation on  silvo_se, control_se using "mice" (Multivariate Imputation by Chained Equations), Upper Quartile, and Mean Imputation
...
col_for_impute <- database_clean_sd |> 
  ...
  select(
    ...
    tree_age, crop_type, tree_type, sub_region, experiment_year, alley_width, silvo_n, control_n,
...
impute_data <- function(data, method_name) {
  if (method_name == "pmm") {
    ...
    upper_quartile_variance <- data %>%
      # The 75th percentile represents a value higher than the median, ensuring that the imputed variances are not unrealistically small while 
...
plot_data <- bind_rows(combined_plot_data)
...
comparison_gt <- prepared_data_gt %>%
  ...
  select(
    metric, category, original, upper_quartile, upper_quartile_relative, pmm, pmm_relative,  pmm_best,  pmm_best_relative, mean_imputation, mean_imputation_relative
         ) |> 
...
comparison_gt <- prepared_data_gt %>%
  ...
  data_color(
    ...
    palette = c("#00FF00", "#FFFF00", "#FF0000"), # Green to Yellow to Red
  ...
  fmt_number(
    columns = c(original, pmm, upper_quartile, mean_imputation, pmm_best),
    decimals = 2
  ) %>%
  fmt_number(
    columns = ends_with("_relative"),
...
# Save the non-imputed dataset as RDS
saveRDS(non_imp_dataset_dummy,
        file = file.path(output_dir, "non_imp_dataset_dummy.rds"))
...
non_imp_data_prep_dummy <- prep_dataset_for_rom(non_imp_dataset_dummy)
```
...
non_imp_data_rom_dummy <- calculate_effect_sizes(non_imp_data_prep_dummy)
```
...
## DUMMY NON-IMPUTED
saveRDS(non_imp_data_rom_dummy,
        file = here::here(output_dir, "non_imp_data_rom_dummy.rds"))
