...
for (response in names(v_matrices)) {
  ...
  model_results[[response]] <- list(
    null = fit_model(data_subset, response, v_matrix, moderators = NULL, intercept = TRUE),
    fixed_no_intercept = fit_model(data_subset, response, v_matrix, moderators, intercept = FALSE),
    random_effects = fit_model(data_subset, response, v_matrix, moderators, random_effects = ~ 1 | exp_id, intercept = TRUE),
    full_no_intercept = fit_model(data_subset, response, v_matrix, moderators, random_effects = list(~ 1 | id_article/response_variable, ~ 1 | exp_id), intercept = FALSE)
  )
}
...
saveRDS(lapply(model_results, `[[`, "fixed_no_intercept"), file = file.path(output_dir, "fitted_models_fixed_no_intercept.rds"))
saveRDS(lapply(model_results, `[[`, "random_effects"), file = file.path(output_dir, "fitted_models_random_effects.rds"))
saveRDS(lapply(model_results, `[[`, "full_no_intercept"), file = file.path(output_dir, "fitted_models_full_no_intercept.rds"))
