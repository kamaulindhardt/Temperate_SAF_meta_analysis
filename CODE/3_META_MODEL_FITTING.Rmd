...
non_imp_dataset <- non_imp_data_rom %>%
  ...
  relocate(
    ...
    experiment_site, location, final_lat, final_lon, exp_site_loc, experiment_year,
    # Moderators info
...
random_effect_structures <- list(
  ...
  study_and_loc_year = list(~ 1 | id_article,                                       # <------- Slightly lower AIC than the chosen one
                            ~ 1 | experiment_site / experiment_year)
  )

# Other random effects structures to that was tested but not included here are:
# study_loc_year     = list(~ 1 | id_article / location * experiment_year)
# study_and_loc_year = list(~ 1 | id_article, ~ 1 | location / experiment_year)
...
# Last go (01/04-2025) 
# Total time taken: 26.87301 secs
```
...
```{r}
# Saving in new data variable, just to ensure consistency
# WORING ON THE IMPUTED DATASET
meta_data <- imp_dataset
...
data_subset <- data_subset %>%
  mutate(across(c(age_system, crop_type, tree_type, season, soil_texture, no_tree_per_m, tree_height, alley_width,
                  organic, tillage), 
                as.factor))
...
fit_meta_analysis_moderator_models <- function(data_subset, response_variable, v_matrix, random_effects) {
  ...
  interaction_pairs <- list(                                                         # Selected two-way interactions 
    ...
    c("tillage", "organic"),
    # Three-way interaction, tested on all, but only valid and relevant for the 'Crop yield' response variable
    c("crop_type", "tree_height", "alley_width"),
    # Management interactions - also tested as moderators
    c("organic", "tillage") #  (1 = organic, 0 = conventional) and (1 = tillage, 0 = no tillage)
  )
...
for (response in names(v_matrices)) {
  ...
  # Other random effects structures that were tested but not included here are:
...
## Exstracting and assessing the fitted models
...
```{r}
# STEP 1: Define a Generic Contrast Function (using the structure: "anova(res, X=rbind(c(1,0,0),c(0,1,0),c(0,0,1)))")

run_contrast_tests <- function(model_results) {
  contrast_summary_all <- tibble()

  for (response in names(model_results)) {
    cat("\n🔎 Response:", response)

    for (moderator in names(model_results[[response]])) {
      model <- model_results[[response]][[moderator]]

      # Check if it's a valid metafor model
      if (is.null(model) || !inherits(model, "rma.mv")) {
        cat("\n  ⏩ Skipping", moderator, "- Not a model")
        next
      }

      # Extract coefficient names
      coef_names <- rownames(model$b)
      if (length(coef_names) < 2) {
        cat("\n  ⏩ Skipping", moderator, "- Only 1 level")
        next
      }

      # Get all pairwise combinations
      pairwise_contrasts <- combinat::combn(coef_names, 2, simplify = FALSE)

      # Construct the contrast matrix
      contrast_matrix <- lapply(pairwise_contrasts, function(pair) {
      ...
      rownames(contrast_matrix) <- sapply(pairwise_contrasts, \(x) paste(x[1], "-", x[2]))

      # Run Wald-type ANOVA
      contrast_results <- tryCatch({
        anova(model, X = contrast_matrix)
      }, error = function(e) {
        cat("\n  ⚠️ Error for", moderator, ":", e$message)
        return(NULL)
      })
      ...
      if (!is.null(contrast_results)) {
        contrast_tbl <- tibble::tibble(
          ...
          Estimate = contrast_results$estimate,
          SE = contrast_results$se,
          Zval = contrast_results$zval,
          P = contrast_results$pval
        )

        contrast_summary_all <- dplyr::bind_rows(contrast_summary_all, contrast_tbl)
        cat("\n  ✅ Added:", moderator)
      }
    }
  }
  ...
  if (nrow(contrast_summary_all) > 0) {
    contrast_summary_all <- contrast_summary_all %>%
  ...
  return(contrast_summary_all)
}

# STEP 2: Apply to All Moderator Models for All Response Variables

contrast_summary_all <- run_contrast_tests(model_results)

# View top contrasts
contrast_summary_all
```

```{r}
# Explicit pairwise Wald ANOVA function
perform_all_pairwise_contrasts <- function(model, response, moderator) {
  # 1. Validate model
  if (is.null(model) || !inherits(model, "rma.mv")) {
    warning(paste("Model is NULL or not of class rma.mv for", response, "-", moderator))
    return(NULL)
  }
  
  # 2. Extract coefficient names
  coef_names <- rownames(model$b)
  if (length(coef_names) < 2) {
    warning(paste("Fewer than 2 coefficients for", response, "-", moderator))
    return(NULL)
  }

  # 3. Build all pairwise contrast matrix
  pairwise_combos <- combinat::combn(coef_names, 2, simplify = FALSE)
  contrast_matrix <- lapply(pairwise_combos, function(pair) {
    vec <- setNames(rep(0, length(coef_names)), coef_names)
    vec[pair[1]] <- 1
    vec[pair[2]] <- -1
    vec
  ...
  rownames(contrast_matrix) <- sapply(pairwise_combos, \(x) paste(x[1], "-", x[2]))

  # 4. Run Wald-type test
  contrast_test <- tryCatch({
    anova(model, X = contrast_matrix)
  }, error = function(e) {
    warning(paste("Failed ANOVA for", response, "-", moderator, ":", e$message))
    return(NULL)
  })

  # 5. Output as tibble
  if (!is.null(contrast_test)) {
    tibble::tibble(
      Response = response,
      Moderator = moderator,
      Contrast = rownames(contrast_matrix),
      Estimate = contrast_test$estimate,
      SE = contrast_test$se,
      Zval = contrast_test$zval,
      P = contrast_test$pval
    ) %>%
...
```{r}
# Test Biodiversity ~ soil_texture
mod <- model_results$Biodiversity$soil_texture

# Run the generic pairwise contrast evaluation
contrast_output <- perform_all_pairwise_contrasts(
  model = mod,
  response = "Biodiversity",
  moderator = "soil_texture"
)

contrast_output
```

From the Wald-type ANOVA contrast test on the soil_texture moderator in the Biodiversity model, none of the pairwise comparisons between soil types show statistically significant differences. Here's how to interpret each contrast:
  - Clay vs Sand: The estimate is close to zero with a p-value of 0.925, indicating no difference in biodiversity outcomes between Clay and Sand soils.
  - Clay vs Silt: The contrast is also non-significant (p = 0.768). Although the estimate is slightly positive, it is well within the margin of error, suggesting no meaningful difference in biodiversity effects between Clay and Silt.
  - Sand vs Silt: Likewise, this comparison is not significant (p = 0.697). The small positive estimate suggests a slight tendency for Silt soils to support stronger biodiversity outcomes than Sand, but this trend is not statistically reliable.
Conclusion:
Across all three contrasts, there is no statistically significant difference in biodiversity effects of silvoarable systems across the soil textures Clay, Sand, and Silt. The estimates are small, and the confidence intervals are wide, indicating that soil texture — at least in this model — does not significantly modify biodiversity outcomes.

```{r}
# Test Crop yield ~ soil_texture
mod <- model_results$`Crop yield`$soil_texture
...
contrast_output
```
The results above compare how crop yield responds to different soil textures under silvoarable systems. The contrasts test whether the average effect sizes differ between each pair of soil types:
  - Clay vs Sand: The estimated difference in crop yield effects is very small and non-significant (p = 0.707). This suggests that Clay and Sand soils do not differ meaningfully in how they influence yield outcomes under agroforestry.
  - Clay vs Silt: This comparison is marginally non-significant (p = 0.061), with a positive estimate. This hints that Silt soils may support slightly better yield outcomes than Clay, but the evidence isn't strong enough to be statistically conclusive.
  - Sand vs Silt: This contrast is statistically significant (p = 0.038). The positive estimate means that Silt soils lead to stronger crop yield benefits under silvoarable systems compared to Sand soils. This finding is statistically reliable at the 5% level.
Conclusion:
Among the soil textures tested, Silt soils appear to outperform Sand in terms of crop yield responses to agroforestry, while differences involving Clay are less clear or not statistically significant. This suggests that soil texture can influence yield outcomes, and Silt may offer more favorable conditions for crop production in agroforestry systems.
...
## Extracting and back-transforming model evaluation results (moderator interaction models)
...
OBS! Needs updating after the new random effects structure has been implemented - "random_effects <- list(~ 1 | id_article / experiment_site * experiment_year)"


(OBS: INTERPRETATION BELOW NEEDS REVISION AFTER NEW RANDOM EFFECTS STRUCTURE IMPLEMENTED AND MODEL FITTING RE-RUN WITH MANAGEMENT)
...
model_results$`Crop yield`$base_model$QE
model_results$`Crop yield`$base_model$tau2
