...
Standardize experiment_site information and create unique identifiers.
...
moderators <- c("tree_type", "crop_type", "age_system", "season", 
                "soil_texture", "no_tree_per_m", "tree_height", "alley_width", "organic", "tillage")
...
random_effect_structures <- list(
  ...
  study_loc_year     = list(~ 1 | id_article / experiment_site * experiment_year),  # <------- ! Chosen Random Effects Structure !
  study_and_loc_year = list(~ 1 | id_article,
                            ~ 1 | experiment_site / experiment_year)
...
# Define moderators
moderators <- c("tree_type", "crop_type", "age_system", "season", "soil_texture", "no_tree_per_m", "tree_height", "alley_width", "organic", "tillage")
...
# Last go (01/04-2025)
# Total time taken: 24.03345 secs 
```
## Best Random Effects Structure Per Response Variable (based on AIC)

```{r}
# Mean AIC per Random Effects Structure
...
# Count how many times each random structure was best
overall_best_random_structure <- best_random_structure_by_response %>%
  count(random_structure, sort = TRUE)

# Compute mean AIC across all models for each random structure
...
# View results
best_random_structure_by_response
overall_best_random_structure
mean_aic_overall
```
...
data_subset <- data_subset %>%
  mutate(across(c(age_system, crop_type, tree_type, season, soil_texture,
                  no_tree_per_m, tree_height, alley_width, organic, tillage), as.factor))
...
fit_meta_analysis_moderator_models <- function(data_subset, response_variable, v_matrix, random_effects) {
  ...
  results$organic <- tryCatch({
    rma.mv(yi, V = v_matrix, mods = ~ -1 + organic, random = random_effects,           # Organic
           data = data_subset, method = "REML", control = control_params)
  }, error = function(e) NULL)
  
  results$tillage <- tryCatch({
    rma.mv(yi, V = v_matrix, mods = ~ -1 + tillage, random = random_effects,           # Tillage
           data = data_subset, method = "REML", control = control_params)
  }, error = function(e) NULL)
  ...
  interaction_pairs <- list(                                                         # Selected two-way interactions 
    ...
    c("organic", "tillage") # <- organic (1 = organic, 0 = conventional; tillage (1 = tillage, 0 = no tillage)
  )
...
for (response in names(v_matrices)) {
  ...
  random_effects <- list(~ 1 | id_article / location * experiment_year)   # experiment_site
...
saveRDS(model_results, file = file.path(output_dir, "fitted_models_all_new_2.rds"))

# Save individual model results per response variable
for (response in names(model_results)) {
  saveRDS(model_results[[response]],
          file = file.path(output_dir, paste0("fitted_models_", response, "_new_2.rds")))
}
...
```{r}
# Cross-checking data
# table(meta_data$id_article, meta_data$experiment_site, meta_data$experiment_year)
# table(meta_data$experiment_site, meta_data$experiment_year)
```
...
saveRDS(evaluation_results, file = file.path(eval_output_dir, "evaluation_results_combined_2.rds"))

for (response in names(evaluation_results)) {
  saveRDS(evaluation_results[[response]],
          file = file.path(eval_output_dir, paste0("evaluation_results_2_", response, ".rds")))
}
...
evaluation_results$`Crop yield`$moderators$alley_width
```


# STEP 6 SENSITIVITY ANALYSIS OF THE MULTI-MODEL FITTING (IMPUTED VS. NON-IMPUTED DATASET)

```{r}
imp_filtered_sens <- imp_dataset %>% filter(response_variable %in% response_variable)
non_imp_filtered_sens <- non_imp_dataset %>% filter(response_variable %in% response_variable)

imp_filtered_sens |> glimpse()
non_imp_filtered_sens |> glimpse()
```

## Fit Base Models (Intercept-only) for Comparison
...
# Function to fit only base model - ("base model" the multilevel model with intercept only)
fit_base_model_sensitivity <- function(data, response, v_matrix, random_effects) {
  tryCatch({
    rma.mv(
      yi = yi,
      V = v_matrix,
    ...
    message("Error for ", response, ": ", e$message)
    NULL
  })
}

# Get the list of response variables to loop over
response_variables <- unique(imp_filtered_sens$response_variable)
...
for (response in response_variables) {
  cat("Fitting base models for:", response, "\n")
  
  imp_data <- imp_filtered_sens %>% filter(response_variable == response)
  non_imp_data <- non_imp_filtered_sens %>% filter(response_variable == response)

  imp_v <- calculate_v_matrix(imp_data)
  non_imp_v <- calculate_v_matrix(non_imp_data)

  random_effects <- list(~ 1 | id_article / location * experiment_year)   # experiment_site --> selected random effects structure

  imp_model <- fit_base_model_sensitivity(imp_data, response, imp_v, random_effects)
  non_imp_model <- fit_base_model_sensitivity(non_imp_data, response, non_imp_v, random_effects)

  sensitivity_models[[response]] <- list(
    imputed = imp_model,
    non_imputed = non_imp_model
  )
}
...
```{r}
compare_results <- function(models, response) {
  imp <- models$imputed
  non_imp <- models$non_imputed

  tibble(
    response_variable = response,
    source = c("Imputed", "Non-Imputed"),
    ROM = c(if (!is.null(imp)) exp(imp$b), if (!is.null(non_imp)) exp(non_imp$b)),
    ci.lb = c(if (!is.null(imp)) exp(imp$ci.lb), if (!is.null(non_imp)) exp(non_imp$ci.lb)),
    ci.ub = c(if (!is.null(imp)) exp(imp$ci.ub), if (!is.null(non_imp)) exp(non_imp$ci.ub)),
    tau2 = c(if (!is.null(imp)) sum(imp$tau2), if (!is.null(non_imp)) sum(non_imp$tau2)),
    k = c(if (!is.null(imp)) imp$k, if (!is.null(non_imp)) non_imp$k),
    AIC = c(if (!is.null(imp)) AIC(imp), if (!is.null(non_imp)) AIC(non_imp)),
    BIC = c(if (!is.null(imp)) BIC(imp), if (!is.null(non_imp)) BIC(non_imp))
  )
}

sensitivity_summary <- purrr::map_dfr(names(sensitivity_models), function(response) {
  compare_results(sensitivity_models[[response]], response)
})
...
```{r}
ggplot(sensitivity_summary, aes(x = source, y = ROM, fill = source)) +
  geom_col(position = position_dodge(), width = 0.6) +
  geom_errorbar(aes(ymin = ci.lb, ymax = ci.ub), width = 0.2) +
  facet_wrap(~ response_variable, scales = "free_y") +
  labs(
    title = "Sensitivity Analysis of Imputation Methods",
    y = "Response Ratio (ROM)",
    x = "Dataset"
  ) +
  theme_minimal()
```
...
```{r}
# AIC
ggplot(sensitivity_summary, aes(x = source, y = AIC, fill = source)) +
  geom_col(position = position_dodge(), width = 0.6) +
  facet_wrap(~ response_variable, scales = "free_y") +
  labs(
    title = "AIC Comparison: Imputed vs. Non-Imputed Models",
...
# BIC
ggplot(sensitivity_summary, aes(x = source, y = BIC, fill = source)) +
  geom_col(position = position_dodge(), width = 0.6) +
  facet_wrap(~ response_variable, scales = "free_y") +
  labs(
    title = "BIC Comparison: Imputed vs. Non-Imputed Models",
...
```{r}
# Assuming your long-format data is stored in `sensitivity_summary`
aic_bic_summary <- sensitivity_summary %>%
  select(response_variable, source, AIC, BIC) %>%
  pivot_wider(
    names_from = source,
...
aic_bic_summary %>%
  gt() %>%
  tab_header(
    title = "Model Fit Comparison: AIC and BIC (Imputed vs. Non-Imputed)"
  ) %>%
  cols_label(
    AIC_Imputed = "AIC (Imputed)",
    `AIC_Non-Imputed` = "AIC (Non-Imputed)",
    BIC_Imputed = "BIC (Imputed)",
    `BIC_Non-Imputed` = "BIC (Non-Imputed)",
    response_variable = "Agroecosystem Service"
  ) %>%
  fmt_number(
    columns = everything(),
    decimals = 1
...
aic_bic_summary %>%
  gt() %>%
  # same tab_header, cols_label etc. as above
  data_color(
    columns = c(AIC_Imputed, AIC_Non_Imputed),
    colors = scales::col_numeric(
      palette = c("white", "#DDEBF7"),
      domain = range(c(aic_bic_summary$AIC_Imputed, aic_bic_summary$AIC_Non_Imputed))
    )
  ) %>%
  data_color(
    columns = c(BIC_Imputed, BIC_Non_Imputed),
    colors = scales::col_numeric(
      palette = c("white", "#FCE4D6"),
      domain = range(c(aic_bic_summary$BIC_Imputed, aic_bic_summary$BIC_Non_Imputed))
    )
  )
...
evaluation_results$`Crop yield`$interactions

evaluation_results$Biodiversity$base_model
evaluation_results$Biodiversity$moderators
evaluation_results$Biodiversity$interactions

evaluation_results$`Carbon sequestration`$base_model
evaluation_results$`Carbon sequestration`$moderators
evaluation_results$`Carbon sequestration`$interactions
```
...
# STEP 8 ANOVA CONTRAST OF MULTI-MODEL EFFECTS ESTIMATES

anova(res, X=rbind(c(1,0,0),c(0,1,0),c(0,0,1)))

```{r}
# Biodiversity - soil_texture
#---------------------------------------------------------------

# STEP 1: Extract the model object
soil_texture_model <- model_results$Biodiversity$soil_texture  # <--- This might be the real model, not a summary
...
```{r}
# Pest and disease control - tree_type
#---------------------------------------------------------------
mod <- model_results$`Pest and disease control`$tree_type
stopifnot(inherits(mod, "rma.mv"))

coef_names <- rownames(mod$b)
print(coef_names)  # Should include: tree_typeBiomass, Fruit,nut & other, Timber

contrast_tree_type <- rbind(
  "Biomass vs Fruit/Nut" = c(1, -1, 0),
  "Biomass vs Timber"    = c(1, 0, -1),
  "Fruit/Nut vs Timber"  = c(0, 1, -1)
)
colnames(contrast_tree_type) <- coef_names

anova(mod, X = contrast_tree_type)
```
```{r}
# Crop yield - crop_type
#---------------------------------------------------------------
mod <- model_results$`Crop yield`$crop_type
stopifnot(inherits(mod, "rma.mv"))

coef_names <- rownames(mod$b)
print(coef_names)  # crop_typeCereal, crop_typeLegume, crop_typeTuber,root and other

contrast_crop_type <- rbind(
  "Cereal vs Legume" = c(1, -1, 0),
  "Cereal vs Tuber"  = c(1, 0, -1),
  "Legume vs Tuber"  = c(0, 1, -1)
)
colnames(contrast_crop_type) <- coef_names

anova(mod, X = contrast_crop_type)
```

```{r}
# Carbon sequestration - age_system
#---------------------------------------------------------------
mod <- model_results$`Carbon sequestration`$age_system
stopifnot(inherits(mod, "rma.mv"))

coef_names <- rownames(mod$b)
print(coef_names)

contrast_age_system <- rbind(
  "Mature vs Medium" = c(0, 1)
)
colnames(contrast_age_system) <- coef_names

anova(mod, X = contrast_age_system)
```

```{r}
# Soil quality - season
#---------------------------------------------------------------
mod <- model_results$`Soil quality`$season
stopifnot(inherits(mod, "rma.mv"))

coef_names <- rownames(mod$b)
print(coef_names)

contrast_season <- rbind(
  "Summer vs Reference" = c(0, 1)  # Assuming intercept is baseline (e.g., Spring)
)
colnames(contrast_season) <- coef_names

anova(mod, X = contrast_season)
```



# STEP 9 BACK-TRANSFORM MODEL RESULTS FROM LOG-ROM TO PERCENTAGE FOR ALL RESPONSE VARIABLES
...
# STEP 10 META-ANALYSIS MODEL DIAGNOSTICS - HETEROGENEITY STATISTICS
...
extract_all_heterogeneity_stats <- function(model_results) {
  ...
  for (response in names(model_results)) {
    ...
    for (model_name in names(models)) {
      ...
      # Handle missing QE or df safely
      if (!is.null(QE) && !is.na(QE) && !is.na(df) && QE > df) {
        I2 <- (QE - df) / QE * 100
      } else {
        I2 <- 0
      }
...
# STEP 11 FOREST PLOTS FOR THE MINIMAL RANDOM EFFECTS MODEL (NOT THE FINAL FOREST PLOT)
...
# STEP 12 EXTRACT MODEL WEIGHTS PER RESPONSE VARIABLE 

 

# STEP 13 SAVING DATASETS AND MODEL OBJECTS
