...
evaluate_model <- function(models, response_variable) {
  ...
  if (!is.null(models$minimal_random_effects)) {
    ...
    cat("\nMinimal random effects model:\n", evaluation_results$minimal_random_effects, "\n")
  }
  ...
  if (!is.null(models$moderator_models)) {
    ...
    for (moderator in names(models$moderator_models)) {
      cat("\nModerator:", moderator, "\n")
      print(evaluation_results$moderator_models[[moderator]])
    }
  }
  ...
  if (!is.null(models$interaction_model)) {
    ...
    cat("\nFull interaction model:\n", evaluation_results$interaction_model, "\n")
  }
...
saveRDS(evaluation_results, file = here::here("DATA", "OUTPUT_FROM_R", "EVALUATION_RESULTS", "evaluation_results_combined.rds"))

# Save individual evaluation results
for (response in names(evaluation_results)) {
  saveRDS(evaluation_results[[response]], file = here::here("DATA", "OUTPUT_FROM_R", "EVALUATION_RESULTS", paste0("evaluation_results_", response, ".rds")))
}

cat("\nEvaluation results have been saved successfully!\n")
```
...
extract_model_stats <- function(models, response_variable) {
  ...
  if (!is.null(models$null_model)) {
    stats <- rbind(stats, data.frame(
      Model = "Null Model",
      AIC = models$null_model$fit.stats["AIC"],
      BIC = models$null_model$fit.stats["BIC"],
      LogLik = models$null_model$fit.stats["logLik"],
  ...
  if (!is.null(models$minimal_random_effects)) {
    stats <- rbind(stats, data.frame(
      ...
      AIC = models$minimal_random_effects$fit.stats["AIC"],
      BIC = models$minimal_random_effects$fit.stats["BIC"],
      LogLik = models$minimal_random_effects$fit.stats["logLik"],
      ResponseVariable = response_variable
    ))
  }
    ...
    for (moderator in names(models$moderator_models)) {
      ...
      if (!is.null(mod)) {
        stats <- rbind(stats, data.frame(
          ...
          AIC = mod$fit.stats["AIC"],
          BIC = mod$fit.stats["BIC"],
          LogLik = mod$fit.stats["logLik"],
          ResponseVariable = response_variable
        ))
      }
    }
  ...
  if (!is.null(models$full_model)) {
    stats <- rbind(stats, data.frame(
      Model = "Full Model",
      AIC = models$full_model$fit.stats["AIC"],
      BIC = models$full_model$fit.stats["BIC"],
      LogLik = models$full_model$fit.stats["logLik"],
      ResponseVariable = response_variable
    ))
  }
  ...
  if (!is.null(models$interaction_model)) {
    stats <- rbind(stats, data.frame(
      ...
      AIC = models$interaction_model$fit.stats["AIC"],
      BIC = models$interaction_model$fit.stats["BIC"],
      LogLik = models$interaction_model$fit.stats["logLik"],
      ResponseVariable = response_variable
    ))
  }
...
output_dir <- here::here("DATA", "OUTPUT_FROM_R", "EVALUATION_RESULTS")
write.csv(all_model_stats, file = file.path(output_dir, "model_statistics.csv"), row.names = FALSE)

##########################################################################
# Create publication-ready visualizations for AIC
##########################################################################


# Plot AIC values for all models
plot_aic <- ggplot(all_model_stats, aes(x = fct_reorder(Model, AIC), y = AIC, fill = ResponseVariable)) +
...
# Save the plot
ggsave(filename = file.path(output_dir, "aic_visualization.png"), plot = plot_aic, width = 12, height = 8, dpi = 300)

cat("AIC visualizations and model statistics table have been saved successfully.")
```
...
```{r}
##########################################################################################################################################
# EVALUATE MODEL RESULTS FOR PUBLICATION-READY OUTPUTS
##########################################################################################################################################

# Define a function to extract model statistics and heterogeneity metrics
evaluate_model_statistics <- function(model, response_variable) {
  if (is.null(model)) return(NULL)

  # Extract effect size, confidence intervals, and heterogeneity metrics
...
##########################################################################################################################################
# SECTION 1: OVERALL EFFECTS OF SILVOARABLE AGROFORESTRY SYSTEMS
##########################################################################################################################################

# 1a: Extract overall effect sizes and confidence intervals for the minimal random effects model
overall_effects <- lapply(names(model_results), function(response) {
  evaluate_model_statistics(model_results[[response]]$minimal_random_effects, response)
}) %>% bind_rows()

# Back-transform effect sizes and confidence intervals into percentages
overall_effects <- overall_effects %>%
  mutate(
    Effect_Size_Percentage = (exp(Effect_Size) - 1) * 100,
    CI_Lower_Percentage = (exp(CI_Lower) - 1) * 100,
    CI_Upper_Percentage = (exp(CI_Upper) - 1) * 100
  )

# 1b: Extract heterogeneity statistics for each response variable
heterogeneity_stats <- overall_effects %>%
  select(Response_Variable, I2, Tau2)

# 1c: Assess publication bias using funnel plots and Egger's test
publication_bias <- lapply(names(model_results), function(response) {
  model <- model_results[[response]]$minimal_random_effects
  if (is.null(model)) return(NULL)

  egger_test <- regtest(model, model = "lm")

  list(
    Response_Variable = response,
    Egger_p_value = egger_test$pval,
    Funnel_Plot = funnel(model)
  )
})

# 1d: Conduct leave-one-out sensitivity analysis
sensitivity_analysis <- lapply(names(model_results), function(response) {
  model <- model_results[[response]]$minimal_random_effects
  if (is.null(model)) return(NULL)

  leave_one_out <- leave1out(model)
