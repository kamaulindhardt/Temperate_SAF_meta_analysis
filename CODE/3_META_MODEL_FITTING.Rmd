...
```{r}
##########################################################################################################################################
# LEAVE-ONE-OUT SENSITIVITY ANALYSIS ON STUDY EFFECTS
##########################################################################################################################################
...
conduct_loo_analysis <- function(meta_data, full_model_formula, random_effects_formula) {
  ...
  for (study in unique_studies) {
    ...
    # Refit the full model without the excluded study
    loo_model <- tryCatch({
      rma.mv(
        yi = yi,
        V = vi,
        mods = full_model_formula,
...
# Define the full model formula and random effects structure
full_model_formula <- as.formula("~ tree_type + crop_type + age_system + season + soil_texture")
random_effects_formula <- ~ 1 | exp_id

# Conduct Leave-One-Out (LOO) sensitivity analysis
loo_results <- conduct_loo_analysis(
  meta_data = meta_data,
...
summarize_loo_results <- function(loo_results) {
...
loo_summary <- summarize_loo_results(loo_results)
