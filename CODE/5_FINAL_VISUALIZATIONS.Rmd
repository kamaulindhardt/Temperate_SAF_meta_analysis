...
# STEP 5
##########################################################################################################################################
FOREST PLOT (VIOLIN PLOT) EFFECT SIZES FOR 
##########################################################################################################################################
...
```{r}
evaluation_results$`Water quality`$minimal_random_effects
```
...
back_transformed_results$`Water quality`$minimal_random_effects
...
# Step 1: Compute median effect sizes for ordering
...
# Step 2: Calculate the number of studies (`n_studies`) and number of observations (`obs_count`) per response variable
...
# Step 3: Merge `study_counts` and `structured_summary` into `imp_data_rom_violin`
imp_data_rom_violin <- imp_data_rom %>%
  left_join(study_counts, by = "response_variable") %>%  # ✅ Correct: Merge study_counts
  left_join(structured_summary, by = c("response_variable" = "ResponseVariable"))  # Merge CI & ROM transformed values

# Step 4: Apply the ordered factor to `imp_data_rom_violin`
...
# Step 5: Merge `study_counts` into `structured_summary` to include `obs_count`
structured_summary <- structured_summary %>%
  left_join(study_counts, by = c("ResponseVariable" = "response_variable")) %>%  # ✅ Correct: Merge study_counts here too
  mutate(ResponseVariable = factor(ResponseVariable, levels = median_effects$response_variable))
  ...
  geom_text(
    aes(
      label = paste0(
        ...
        "n studies = ", n_studies.y, " | n obs = ", obs_count.y
      )
    ),
    hjust = 1,
    size = 4
