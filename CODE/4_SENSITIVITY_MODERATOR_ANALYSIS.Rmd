...
response_summary <- lapply(names(response_specific_results), function(response) {
  ...
  full_effect_size <- structured_results %>%
    filter(ResponseVariable == response, Model == "full_model") %>%
    pull(Estimate)
...
post_processed_results_higher_level <- post_processed_results$HigherLevel |> relocate(
  ResponseVariable,
  Moderator,
  Term,
  Estimate,
...
# View summaries
post_processed_results$HeterogeneityExplained
post_processed_results_higher_level |> str()

post_processed_results_higher_level 
```

```{r}
# Visualize ProportionExplained for Overall and Response Variable-Specific Situations
...
# Generate plots
plots <- visualize_proportion_explained(post_processed_results)
