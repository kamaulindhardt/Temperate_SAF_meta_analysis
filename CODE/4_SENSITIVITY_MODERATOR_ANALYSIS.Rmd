...
# Last go (20/8-2025)
# Total time taken: 10.87276 mins 
```
...
## Stacked Bar Plot of PEH Contributions

```{r}
# Ensure correct factor levels for ordered display
bootstrap_results_df <- bootstrap_results_df %>%
  mutate(
    ResponseVariable = factor(ResponseVariable, levels = unique(ResponseVariable)),
    Moderator = factor(Moderator, levels = unique(Moderator))
  )

# Normalize PEH per response variable to max 100% total (for stacked plot only)
normalized_peh <- bootstrap_results_df %>%
  group_by(ResponseVariable) %>%
  mutate(PEH_Sum = sum(MeanPEH, na.rm = TRUE),
         MeanPEH_Normalized = MeanPEH / PEH_Sum * 100) %>%
  ungroup()

# Stacked bar plot (piled) of PEH by response variable
stacked_peh_plot <- ggplot(normalized_peh, aes(x = ResponseVariable, y = MeanPEH_Normalized, fill = Moderator)) +
  geom_bar(stat = "identity", position = "stack", color = "black", alpha = 0.9) +
  coord_flip() +
  labs(
    title = "Normalized Stacked Proportion of Explained Heterogeneity (PEH)",
...
```{r}
# Define moderators and response variables of interest
moderators_to_keep <- c("tree_type", "crop_type", "season", "alley_width", "age_system", "tree_height")
aes_to_keep <- c("Biodiversity", "Crop yield", "Soil quality")

# Define final level order: specific moderators first, "Others" last
moderator_order <- c(moderators_to_keep, "Others")



# Prepare data
normalized_peh <- bootstrap_results_df %>%
  filter(ResponseVariable %in% aes_to_keep) %>%
  mutate(
    Moderator = as.character(Moderator),
    Moderator = ifelse(Moderator %in% moderators_to_keep, Moderator, "Others"),
    Moderator = fct_relevel(Moderator, moderators_to_keep, after = Inf),  
    ResponseVariable = factor(ResponseVariable, levels = aes_to_keep)
  ) %>%
  group_by(ResponseVariable) %>%
  mutate(
    PEH_Sum = sum(MeanPEH, na.rm = TRUE),
    MeanPEH_Normalized = MeanPEH / PEH_Sum * 100
  ) %>%
  ungroup()

# Collapse all "Others" into one entry per ResponseVariable
normalized_peh_collapsed <- normalized_peh %>%
  group_by(ResponseVariable, Moderator) %>%
  summarise(
    MeanPEH = sum(MeanPEH, na.rm = TRUE),
    PEH_Sum = first(PEH_Sum),
    .groups = "drop"
  ) %>%
  mutate(
    MeanPEH_Normalized = MeanPEH / PEH_Sum * 100
...
# Generate plot
stacked_peh_plot <- ggplot(normalized_peh_collapsed, aes(x = ResponseVariable, y = MeanPEH_Normalized, fill = Moderator)) +
  geom_bar(stat = "identity", position = "stack", color = "black", alpha = 0.9) +
  coord_flip() +
  scale_fill_manual(values = custom_colors, drop = FALSE) +  # Prevent dropping "Others"
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(
    title = "Normalized Proportion of Explained Heterogeneity (PEH)",
    y = "Relative Contribution (%)",
    x = "Agroecosystem Service (AES)",
    fill = "Moderator"
  ) +
  theme_minimal() +
  theme(
...
stacked_peh_plot <- ggplot(normalized_peh_collapsed, aes(x = ResponseVariable, y = MeanPEH_Normalized, fill = Moderator)) +
  ...
  scale_fill_manual(
    ...
    breaks = moderators_to_keep,  # Exclude "Others" from legend
    drop = FALSE
  ) +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(
    title = "Normalized Proportion of Explained Heterogeneity (PEH)",
    y = "Relative Contribution (%)",
    x = NULL  # or use: x = "", or leave out to keep default
  ) +
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 12),
...
# Prepare data: Filter only selected moderators and AES
filtered_peh <- bootstrap_results_df %>%
  filter(
    ResponseVariable %in% aes_to_keep,
    Moderator %in% moderators_to_keep
  ) %>%
  mutate(
    Moderator = factor(Moderator, levels = moderators_to_keep),
    ResponseVariable = factor(ResponseVariable, levels = aes_to_keep)
  )
...
# Generate stacked bar plot without normalization
stacked_peh_plot <- ggplot(filtered_peh, aes(x = ResponseVariable, y = MeanPEH, fill = Moderator)) +
  geom_bar(stat = "identity", position = "stack", color = "black", alpha = 0.9) +
  coord_flip() +
  scale_fill_manual(values = custom_colors, drop = FALSE) +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(
    title = "Proportion of Explained Heterogeneity (PEH)",
    y = "Proportion Explained (%)",
    x = NULL,  # Remove y-axis title
    fill = "Moderator"
  ) +
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 12),
    axis.title.y = element_blank(),  # Ensure y-label is removed
    axis.title.x = element_text(size = 13),
    plot.title = element_text(size = 14, face = "bold"),
    legend.position = "right"
  )
