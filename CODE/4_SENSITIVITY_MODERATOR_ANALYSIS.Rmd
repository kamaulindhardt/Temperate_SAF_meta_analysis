...
```{r}
# Split data and V_matrix by response variable with preselection
...
```{r}
run_loo_sensitivity <- function(data, V_matrix, moderator_formula, random_effects = NULL, output_dir = NULL, transf = NULL) {
  # Ensure the V_matrix is positive definite
  V_matrix <- make_positive_definite(V_matrix)
  
  # Combine moderator_formula and random_effects
  full_formula <- if (!is.null(random_effects)) {
    as.formula(paste(deparse(moderator_formula), deparse(random_effects), sep = " + "))
  } else {
    moderator_formula
  }
...
```{r}
# Define the function to run and save LOO sensitivity results
run_loo_sensitivity_for_splits <- function(splits, formula, random_effects = NULL, output_dir = NULL) {
...
split_by_response_variable <- function(test_data, selected_response_variables = NULL) {
  ...
  if (!is.null(selected_response_variables)) {
    ...
    V_matrix <- V_matrix[rownames(data), rownames(data), drop = FALSE]
  }
...
```
```{r}
# Print response variables with missing models
missing_models <- purrr::map(names(results$non_imp), function(response_var) {
  if (is.null(results$non_imp[[response_var]]$model)) {
    response_var
  }
}) %>% purrr::compact()

cat("Response variables without valid models:\n", missing_models)
```

```{r}
visualize_moderator_response <- function(results, moderator, response_variable) {
  extract_models <- function(results_group) {
    map(results_group, ~ list(
      model_object = .x$model,
      response_variable = .x$response_variable
    ))
  }
  
  all_models <- c(
    extract_models(results$non_imp),
    extract_models(results$imp),
    extract_models(results$non_imp_imputed),
    extract_models(results$imp_imputed)
  )
  
  matching_models <- keep(all_models, ~ .x$response_variable == response_variable)
  
  if (length(matching_models) == 0) {
    stop(paste("No models found for response variable:", response_variable))
  }
  
  selected_model <- NULL
  for (model_info in matching_models) {
    predictors <- rownames(model_info$model_object$beta)
    if (any(grepl(moderator, predictors, ignore.case = TRUE))) {
  ...
  if (is.null(selected_model)) {
    available_predictors <- unique(unlist(lapply(matching_models, function(m) rownames(m$model_object$beta))))
    stop(
      paste0(
        "No model found with moderator matching: '", moderator, "'.\n",
        "Available predictors: ", paste(available_predictors, collapse = ", ")
      )
    )
  }
  
  # Debugging: Check selected_model structure
  message("Selected model structure:")
  print(str(selected_model$model_object))
  
  # Extract forest plot data
  forestplot_data <- extract_forestplot_data(selected_model$model_object)
  
  # Generate plot
  plot <- plot_forestplot(forestplot_data, title = paste("Forest Plot:", moderator, "vs", response_variable))
  return(plot)
}
...
```{r}
extract_forestplot_data <- function(model_object) {
  # Validate structure
  if (!is.list(model_object) || !is.matrix(model_object$beta) || is.null(rownames(model_object$beta))) {
    stop("The input model_object does not have the expected structure.")
  }
  
  # Extract components
  beta <- model_object$beta[, 1]  # Extract estimates
  se <- model_object$se           # Extract standard errors
  ci.lb <- model_object$ci.lb     # Lower bounds of confidence intervals
  ci.ub <- model_object$ci.ub     # Upper bounds of confidence intervals
  pval <- model_object$pval       # P-values
  term_names <- rownames(model_object$beta)  # Predictor names

  # Combine into a tibble for easy manipulation
...
```{r}
forestplot_data <- extract_forestplot_data(results$non_imp[[1]])

# Check the structure of the resulting tibble
# str(forestplot_data)
forestplot_data |> glimpse()

results$non_imp[[1]]$response_variable
```


```{r}
forestplot_data |> 
  ggplot(aes(x = term, y = estimate, ymin = ci_lower, ymax = ci_upper)) +
  geom_pointrange() +
  coord_flip() +
  theme_minimal() +
  labs(title = "Forest Plot", y = "Effect Estimate", x = "Predictors")
```
```{r}
# Visualize forest plot for a specific moderator and response variable
forest_plot <- visualize_moderator_response(
  results = results,
  moderator = "crop_type",  # Match is case-insensitive and partial
  response_variable = "Soil quality"
)
...
```
```{r}
combine_forestplot_data <- function(results, moderator) {
  extract_models <- function(results_group) {
    map(results_group, ~ list(
      model_object = .x$model,
      response_variable = .x$response_variable
  ...
  combined_data <- map_dfr(all_models, function(model_info) {
    model_object <- model_info$model_object
    response_variable <- model_info$response_variable
    
    # Skip if no matching predictors
    term_names <- rownames(model_object$beta)
    if (!any(grepl(moderator, term_names, ignore.case = TRUE))) {
      return(NULL)
    }
    
    # Extract the data
    forestplot_data <- extract_forestplot_data(model_object, moderator)
    
    # Add response variable as a column
    forestplot_data <- forestplot_data %>%
      mutate(response_variable = response_variable)
    
    return(forestplot_data)
  })
  
  if (nrow(combined_data) == 0) {
    stop(paste("No terms found matching moderator:", moderator))
  }
...
```{r}
visualize_forestplot_facet <- function(results, moderator) {
  # Combine data across all models
  combined_data <- combine_forestplot_data(results, moderator)
  
  # Create the plot
  plot <- combined_data %>%
    ggplot(aes(x = estimate, y = term, xmin = ci_lower, xmax = ci_upper, color = response_variable)) +
    geom_pointrange(position = position_dodge(width = 0.5)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
    facet_grid(rows = vars(term), cols = vars(response_variable), scales = "free", space = "free") +
    theme_minimal() +
    labs(
...
```{r}
forest_plot <- visualize_forestplot_facet(
  results = results,
  moderator = "crop_type"  # Specify the moderator to visualize
)
