...
cooks_values <- cooks.distance(res_meta, progbar = TRUE)
dfbetas_values <- dfbetas(res_meta, progbar = TRUE)
hat_values <- hatvalues(res_meta, type = "diagonal")

# Convert DFBETAS matrix to a data frame
dfbetas_df <- as.data.frame(dfbetas_values)
dfbetas_df$Study <- meta_data$id_article  # Add study identifiers

# Convert results to a single dataframe for visualization
