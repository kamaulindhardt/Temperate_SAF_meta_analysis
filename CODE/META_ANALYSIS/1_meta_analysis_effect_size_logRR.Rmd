...
# Unfortunately, the details of moderators were specified in another updated datasheet under another tab called "descriptive data" -- which is not 
# the same tab used for the quantitative meta-analysis modelling and diagnostics. Hence, it is necessary to first import that new sheet and merge the details 
# with the previous version (as I have ran all other models and diagnostics with the previous version of the dataset.), and then make sure information from the "descriptive data" sheet match 1:1 with the qualitative, on a study-observation level.  

# List of moderators to investigate
moderators <- c("tree_crop_combination_t", "alley_width")

# All moderators, that needs update and merging of the sheets "descriptive" and "quantitative" before proceeding
# ("soil_texture", "tree_crop_combination_t", "season", 
...
moderator_model <- rma.mv(
  ...
  data = filtered_meta_data_ROM,  
  # Restricted Maximum Likelihood (REML) for estimating variance components, reducing bias.
  method = "REML",               
  control = list(
  # Use the nlminb optimizer to handle complex random-effects models.
    optimizer = "nlminb",     
  ...
  # Enable progress information and convergence diagnostics during model fitting.
  verbose = TRUE                
)
...
readr::write_csv(moderator_results, 
                 here::here("C:/Users/au759124/OneDrive - Aarhus universitet/Documents/Temp_SAF_meta_analysis/DATA/META_ANALYSIS/FROM_R/moderator_results_3.csv"))
...
# In this code, we first create a formula for the moderators by concatenating the list of variables into a single formula. This formula is passed into the mods
# argument in the rma.mv() function to include the moderators in the meta-analysis model. The model itself is fitted using the rma.mv() function from the metafor
# package. It estimates the effect sizes for each moderator while accounting for random effects at the article level and the response variable level.
 
# The model uses Restricted Maximum Likelihood (REML) to estimate the variance components, as REML is effective at reducing bias when estimating random effects.
# To ensure convergence, the optimizer is set to nlminb, with a maximum iteration count of 5000 and a relative tolerance of 1e-5, which balances precision and
# computational feasibility. The progress and diagnostics of the model fitting process are also displayed, as verbose = TRUE.

# After fitting the model, we extract key results such as the coefficients (effect sizes), standard errors, confidence intervals, and p-values for each moderator.
# These results are stored in a data frame, and the moderator names are added for clarity. The results are then saved to a CSV file for further review and 
# analysis.

# Finally, the model results are printed to the console for immediate inspection, and the total time taken to fit the model is calculated and recorded. This
# process allows for a comprehensive assessment of how each moderator affects the outcome, contributing valuable insights into the meta-analysis.
