...
4.  To what extend does pedoclimatic properties, such as soil quality, temperature and precipitation drive inter-site differences? 
...
**Defining colour pallettes for figures**
...
# Silencing any potential warnings
suppressWarnings({ 

# using the here::here function to load the data from file path  
database <- read_excel(path = here::here("DATA/DATABASE_2.0.xlsx"), 
...
**Here we load the datasets prepared in the file "1_loading_and_preparing_data"**
...
pub.year.plot <-
  ...
  theme_bw() +
  scale_x_continuous(breaks = pretty(pub.year.data$PUB.YR, n = 25), 
                     minor_breaks = 1, limits = c(1990, 2025)) +
...
webr::PieDonut(temporal.scale.PD.data,
  aes(Tier_1_scale, count = n), #aes(Tier_1_scale, Tier_2_scale, count = n),
  ratioByGroup = TRUE,
  showPieName = FALSE,
  addDonutLabel = FALSE,
...
system.trait.plot <-
  ...
  geom_text(aes(label = paste0(round(pct * 100, 1), "%")), size = 5, vjust = -1) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none",
...
```{r}
ggsave(
  filename = "system.trait.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 25, 
  height = 15, 
...
```{r}
database %>% 
  dplyr::select(ID.S, ID.P,
                TREE.AGE, TREE.AGE.MIN, TREE.AGE.MAX) %>%
  mutate_if(is.character, as.numeric) %>%
  # Where there is NA, calculating a TREE.AGE based on averages of TREE.AGE.MIN and TREE.AGE.MAX
  dplyr::mutate(TREE.AGE_avr = (TREE.AGE.MIN + TREE.AGE.MAX)/2) %>%
  dplyr::mutate(TREE.AGE_new = coalesce(TREE.AGE, TREE.AGE_avr)) %>%
  dplyr::select(-(TREE.AGE_avr)) %>%
...
**Biodiversity next to ES**
```{r}
biodiversity.ES.studies <- 
  database %>%
  filter(INCLUDED == TRUE) %>%
  # selecting Biodiversity columns
  dplyr::select(
    ID.P, ID.S, AUTHOR, TITLE, DOI, PUB.YR, REVIEWER.NAME, STUDY.LOC.COUNTRY,
    starts_with("BD"),
    starts_with("ES")
  ) %>%
  mutate(across(matches("BD"), as.logical)) %>%
  ...
  mutate_if(is.logical, as.numeric) %>%
   # including NA's as 0 (FALSE)
  mutate_at(vars(-c(ID.P, ID.S, AUTHOR, TITLE, DOI, PUB.YR, REVIEWER.NAME, STUDY.LOC.COUNTRY)),
            tidyr::replace_na, 0) %>%
  mutate_at(c("ID.P", "ID.S"), as.numeric) %>%
  # filter all recorded cases where biodiversity has been assessed together with ES
  filter(BD == 1) %>%
  distinct(ID.P, AUTHOR, TITLE, DOI, PUB.YR, REVIEWER.NAME, STUDY.LOC.COUNTRY,
           across(contains("BD")),
           across(contains("ES"))) # including only independent studies and not separate study sites

  
openxlsx::write.xlsx(biodiversity.ES.studies, "biodiversity.ES.studies.xlsx")
```
...
```{r }
total_n_ES <- vroom::vroom("./DATASET.FROM.SCRIPT/total_n_ES.csv") 
...
**Percentage of various ES representation** 
Calculating the percentage of various ES representation and isolating total n of ES to be used in plotting
...
*Number of papers reporting on management/labour/mechanisation*
```{r}
category.ecosystem.long %>%
  dplyr::filter(name == "Mechanisation" &
                  value == 1) %>%
  distinct()
```
...
## Study sites - continents

*Distribution of study sites across continents*
```{r}
database %>%
  filter(INCLUDED == TRUE) %>%
  
  dplyr::select(ID.P, ID.S, STUDY.CONTINENT) %>%
  distinct() %>%
  group_by(STUDY.CONTINENT) %>%
  #filter(STUDY.CONTINENT == "Europe") %>%
  summarise(count = n()) %>%
  mutate(pct_of_study_sites_per_Continent = count / n_individual_study_sites * 100) %>%
...
*Distribution of studies across continents - Europe*
```{r}
database %>%
  filter(INCLUDED == TRUE) %>%
  
  dplyr::select(ID.P, STUDY.CONTINENT) %>%
  distinct() %>%
  group_by(STUDY.CONTINENT) %>%
  #filter(STUDY.CONTINENT == "Europe") %>%
  summarise(count = n()) %>%
  mutate(pct_of_study_sites_per_Continent = count / n_independent_studies * 100) %>%
  mutate(across(where(is.numeric), round, 1)) %>%
  drop_na() %>%
  na.omit() %>%
  arrange(desc(count))
...
## SAF system trait types - countries

**Loading data**
```{r}
SAFT.countries <- vroom::vroom("./DATASET.FROM.SCRIPT/ES.countries.crops.SAFT.csv") 
```

**Most represented countries**
```{r}
rep.countries <- SAFT.countries %>%
  dplyr::select(ID.P, ID.S, STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  distinct() %>%
  group_by(STUDY.LOC.COUNTRY) %>%
  summarise(count = n()) %>%
  mutate(pct_of_total_study_sites = count / n_individual_study_sites * 100) %>%
  mutate(across(where(is.numeric), round, 1)) %>%
  arrange(desc(count)) 

# -----------------------------------------------------------
# converting dataframe (tibble) to a flextable class 
# (https://ardata-fr.github.io/flextable-book/)

rep.countries.ft <- flextable(rep.countries %>%
  arrange(desc(count)))

# setting headers
rep.countries.ft <- set_header_labels(rep.countries.ft,
  STUDY.LOC.COUNTRY = "Country",
  count = "n records",
  pct_of_total_study_sites = "% of total study sites"
)

# setting table properties
rep.countries.ft <- set_table_properties(rep.countries.ft,
  layout = "autofit",
  width = 0.6
)
...
rep.countries.ft
```
*Exporting represented countries table into to png*
```{r Exporting the table, warning=FALSE, message=FALSE, eval=FALSE}
# saving / exporting the flextable to local disk as ".png"
flextable::save_as_image(rep.countries.ft, path = "./FIGURES.OUTPUT.FROM.SCRIPT/rep.countries.png")
```

*Custom factor level ordering of countries*
```{r}
# custom factor level ordering of countries based on the most represented countries
# Poland is not included <-  because only top 10 is used
custom_countries_order <- c(
  "Belgium",
  "France",
  "Germany",
  "Spain",
...
## SAF system trait types - crop groups

**Distribution of SAF system trait types and crop groups**
```{r}
SAFT.countries %>%
  dplyr::select(ID.P, ID.S, CROP.OUTPUT.GROUP, `SAF System Trait type`) %>%
  distinct() %>%
  ...
  mutate_if(is.character, as.factor) %>%
  arrange(CROP.OUTPUT.GROUP, desc(count))
```

**Distribution of countries and SAF system trait types**
```{r}
SAFT.countries %>%
  dplyr::select(ID.P, ID.S, STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  distinct() %>%
  group_by(STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  drop_na() %>%
  na.omit() %>%
  summarise(count = n()) %>%
  # arrange(desc(count)) %>%
  ...
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  arrange(STUDY.LOC.COUNTRY, desc(count))
```

*Distribution of SAF system trait types across individual countries - USA*
```{r}
SAFT.countries %>%
  dplyr::select(ID.P, ID.S, STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  distinct() %>%
  group_by(STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  filter(STUDY.LOC.COUNTRY == "USA") %>%
  summarise(count = n()) %>%
  mutate(pct_of_studies_USA = count / sum(count) * 100) %>%
  mutate(across(where(is.numeric), round, 1)) %>%
  drop_na() %>%
  na.omit() %>%
  arrange(desc(count))
```
*Distribution of SAF system trait types across individual countries - Belgium*
```{r}
SAFT.countries %>%
  dplyr::select(ID.P, ID.S, STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  distinct() %>%
  group_by(STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  filter(STUDY.LOC.COUNTRY == "Belgium") %>%
  summarise(count = n()) %>%
  mutate(pct_of_studies_USA = count / sum(count) * 100) %>%
  mutate(across(where(is.numeric), round, 1)) %>%
  drop_na() %>%
...
**Matrix; SAF system trait types - countries**
```{r PLOTTING - selected countries and the SAF systems represented, warning=FALSE, message=FALSE}
systrait.country.plot <- SAFT.countries %>%
  dplyr::select(ID.P, ID.S, STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  distinct() %>%
  group_by(STUDY.LOC.COUNTRY, `SAF System Trait type`) %>%
  drop_na() %>%
  ...
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  mutate(custom_countries_order = factor(STUDY.LOC.COUNTRY, 
                                          levels = custom_countries_order)) %>%
  arrange(custom_countries_order) %>%
  na.omi() %>%
  
  
  ggplot(aes(
    x = factor(custom_countries_order),
    y = reorder(`SAF System Trait type`, count),
 ...
 theme(
  ...
  labs(
    ...
    # subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
    caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks.")),
    x = "Countries of publication",
    y = "SAF system trait type",
  )
# facet_grid(. ~ sand_interval)

systrait.country.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "systrait.country.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 35, 
  height = 15, 
...
```{r}
ggsave(
  filename = "inoutES.pub.yr.all.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 30, 
  height = 15, 
...
*Custom factor level ordering of ES variables*
```{r}
# custom factor level ordering of ES variables
custom_ES_order <- c(
    # Biodiversity ---------------------------------------------
...
# custom factor level ordering of ES variables for the selected bioclim-related ES
custom_selected_bioclim_ES_order <- c(
    # --------------------------------------------------------------------
#--# Biodiversity ---------------------------------------------
    "Associated biodiversity",
#--# Supporting ecosystem services ----------------------------
...
```{r}
ES.selected.crop.groups.SATF %>%
  filter(CROP.OUTPUT.GROUP == "Vegetable crops" &
           `SAF System Trait type`== "Fruit - Arable" ) %>%
  distinct(ID.P)
```
...
ES.cat.crop.groups.SATF.plot <- ES.selected.crop.groups.SATF %>%
  ...
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  mutate(ES_ordered = factor(ES.GROUPS, levels = custom_selected_bioclim_ES_order)) %>%
  arrange(ES_ordered) %>%
  drop_na(ES_ordered) %>%
  na.omit(ES_ordered) %>%
  ...
  ggplot(aes(
    ...
    y = factor(ES_ordered, level = custom_selected_bioclim_ES_order),
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right",
    plot.margin = margin(l = 0 + margin_spacer(ES.selected.crop.groups.SATF$CROP.OUTPUT.GROUP)),
    text = element_text(size = 18)
  ) +
  scale_y_discrete(limits = rev) +
...
ES.selected.tree.genera.SATF <- vroom::vroom("./DATASET.FROM.SCRIPT/ES.selected.tree.genera.SATF.csv") 
ES_TREE <- vroom::vroom("./DATASET.FROM.SCRIPT/ES_TREE.csv") 
```

**Matrix; selected ecosystem services - tree.genera - functional role (product type) of tree component** of SAF system trait types
```{r PLOTTING - selected ecosystem services (including biodiversity and categories data) with tree.genera, warning=FALSE, message=FALSE}
ES.cat.tree.genera.plot <- ES.selected.tree.genera.SATF %>%
  group_by(ES.GROUPS, TREE.GENUS.GROUP, ST.TYPE.PROD.TYPE) %>% # not `SAF System Trait type`
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  drop_na(ES_ordered) %>%
  na.omit(ES_ordered) %>%
  # removing unspecified trees
  filter(!TREE.GENUS.GROUP == "Unspecified trees") %>%
  # removing the category "mixed" and "nature + wind shelter" remaining with "biomass" and "food"
  filter(ST.TYPE.PROD.TYPE != "Mixed",
         ST.TYPE.PROD.TYPE != "Nature + wind shelter") %>%      
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right",
    plot.margin = margin(l = 0 + margin_spacer(ES.selected.tree.genera.SATF$TREE.GENUS.GROUP)),
    text = element_text(size = 20)
  ) +
  scale_size_continuous(range = c(1, 10)) +
  scale_fill_discrete_qualitative(palette = "Dynamic") +
  scale_y_discrete(limits = rev) +
  guides(size = guide_legend(title = ""), fill = FALSE) +
  labs( # title = "Matrix showing the frequency of associations of tree.genera, ecosystem services for different silvoarable systems reported by peer-reviewed papers",
    # subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
    # caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks. Total number of independent studies reporting on ES = ", total_n_ES$value)),
    caption = paste("n (individual study sites) = ", n_individual_study_sites),
    x = "Silvoarable System Trait Types",
  ...
  facet_grid(. ~ ST.TYPE.PROD.TYPE)

ES.cat.tree.genera.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "ES.cat.tree.genera.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 48, 
  height = 18, 
...
**Matrix; selected ecosystem services - tree genera**
```{r PLOTTING - selected ecosystem services (including biodiversity and categories data) with tree.genera, warning=FALSE, message=FALSE}
ES.tree.genera.plot <- ES_TREE %>%
  group_by(ES.GROUPS, TREE.GENUS.GROUP) %>% # not `SAF System Trait type`
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  # removing unspecified trees
  filter(!TREE.GENUS.GROUP == "Unspecified trees") %>%
  # removing "unspecified trees"NA" ES
  filter(!ES_ordered == "NA") %>%
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right",
    plot.margin = margin(l = 0 + margin_spacer(ES_TREE$TREE.GENUS.GROUP)),
    text = element_text(size = 20)
  ) +
  scale_size_continuous(range = c(1, 10)) +
  scale_fill_discrete_qualitative(palette = "Dynamic") +
...
```{r}
ggsave(
  filename = "ES.tree.genera.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 48, 
  height = 18, 
...
ES.countries.crops.SAFT.plot <- ES.countries.crops.SAFT %>%
  group_by(STUDY.LOC.COUNTRY, ES.GROUPS, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  mutate(ES_ordered = factor(ES.GROUPS, levels = custom_ES_order)) %>%
  arrange(ES_ordered) %>%
  na.omit() %>%
  
  ggplot(aes(
    x = factor(STUDY.LOC.COUNTRY, level = custom_countries_order),
    y = factor(ES_ordered, level = custom_ES_order),
    fill = STUDY.LOC.COUNTRY
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
  ...
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10), legend.position = "right",
    text = element_text(size = 16),
    plot.margin = margin(l = 0 + margin_spacer(ES.countries.crops.SAFT$STUDY.LOC.COUNTRY))
  ) +
  scale_size_continuous(range = c(1, 10)) +
  scale_fill_discrete_qualitative(palette = "Dynamic") +
  guides(size = guide_legend(title = ""), fill = FALSE) +
  ...
  labs(
    ...
    #subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
    caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks. Total number of independent studies reporting on ES = ", total_n_ES$value, "; n (individual study sites) = ", n_individual_study_sites)),
    x = "Countries",
    y = "Selected ecosystem services"
  ) +
...
```{r}
ggsave(
  filename = "ES.countries.crops.SAFT.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 40, 
  height = 15, 
...
**Loading the combined database with location-specific pedoclimatic (bioclim + soil) data**
...
**Line graph plot; relationship between interrow distance - latitude**
...
lat.interrow.plot <-
  ...
  ggdensity::geom_hdr(xlim = c(0, 80), ylim = c(0, 60), alpha = 0.2, size = 2) +
  geom_smooth(method = lm, se = FALSE, colour = "black") + # add linear trend line formula = y ~ splines::bs(x, 3)
  geom_text(x = 35, y = 65, label = lm_eqn(data), parse = TRUE, size = 8, col = "black") +
  theme_classic() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1), legend.position = "none",
    plot.margin = margin(t = 10, r = 0, b = 15, l = 15, unit = "pt"),
...
combined.geolocation.interrow.lat <- as_ggplot(gt) + # transform to a ggplot
  draw_plot_label(
    label = c("A", "B", "C"), size = 15,
    x = c(0, 0, 0.5), y = c(1, 0.5, 0.5)
  ) # Add labels

combined.geolocation.interrow.lat
```
...
**Where and when; combining the two maps (global and Europe**
*Creating different world map without top-bottom borders*
```{r}

world.study.sites.full <-
  ggplot() +
  geom_map(
    data = world, map = world,
    aes(long, lat, map_id = region),
  ...
  geom_rect(
    aes(xmin = -15, xmax = 35, ymin = 35, ymax = 70),
    color = "black", fill = NA, inherit.aes = FALSE
  ) +
  theme_bw() +
  theme(
...
*Creating map of Europe with legend and top-bottom boarders*
```{r}
# We can now plot the map with the specified display window for zoom level 2, highlighting the zoom center for Europe
...
# Add labels to the arranged plots
combined.geolocation.map <- as_ggplot(gt_map) + # transform to a ggplot
  draw_plot_label(
    label = c("a)", "b)"), size = 25,
    x = c(0.18, 0.18), y = c(1, 0.6)
  ) # Add labels

combined.geolocation.map
```

*Saving last plot*
```{r}
ggsave(
  filename = "combined.geolocation.map.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 35, 
  height = 15, 
...
# Add labels to the arranged plots
combined.geolocation.map.2 <- as_ggplot(gt_map.2) + # transform to a ggplot
  draw_plot_label(
    label = c("a)", "b)"), size = 25,
    x = c(0.0, 0.60), y = c(0.9, 0.9)
  ) # Add labels

combined.geolocation.map.2
```

*Saving last plot*
```{r}
ggsave(
  filename = "combined.geolocation.map.2.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 30, 
  height = 15, 
...
## Spatial arrangements (interrow distance/width) - study site location (latitude)

```{r}
# lat.interrow.plot               # (created further up)
# spatial.arrange.ridge.plot      # (created further up)
```

*Combining two plots; ridgeline plot of interrow distance for different SAF systems and line graph latitude*
```{r}
# Arrange plots using arrangeGrob and using gridExtra and cowplot
# returns a gtable (gt)
gt_interrow <- arrangeGrob(
  spatial.arrange.ridge.plot, 
  lat.interrow.plot,
  ncol = 5,
  nrow = 1,
...
# Add labels to the arranged plots
combined.interrow <- as_ggplot(gt_interrow) + # transform to a ggplot
  draw_plot_label(
    label = c("a)", "b)"), size = 30,
    x = c(0, 0.59), y = c(1.0, 1.0)
  ) # Add labels
...
```{r}
ggsave(
  filename = "combined.interrow.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 35, 
  height = 20, 
...
To answer the question; **does SAF, ES research focus differ as related to location-specific pedoclimatic variables**

Various pedoclimatic variables are **discretised** (from numeric biophys into categorical groups). Adding grouped intervals based on continuous soil variables. This is in order to visualise them in relation to ES for instance.

**Loading and preparing dataset** The dataset is including only *selected ecosystem services*

To form the selected ecosystem services we use information from the *data dimensions*; **ecosystem services**, **biodiversity** and **categories of approach**, following the style of *Ditzler et al. (2021)*, *Duru et al. (2015)*, *Beillouin et al. (2021)* and *Tamburini et al. (2020)*
...
biophys.discretised.data <- vroom::vroom("./DATASET.FROM.SCRIPT/biophys.discretised.data.csv") %>%
  dplyr::select(
    ...
    "lon",
# ----------------------------------------------------------------------------------------------------------
# These selected ecosystem services (below) are based on the *data dimensions*; **ecosystem services**, **biodiversity** and **categories of approach**. Following the style of *Ditzler et al. (2021)*, *Duru et al. (2015)* Beillouin et al. (2021) and *Tamburini et al. (2020)* 
...
# ------------------------------------------------------------------ pedoclimatic (soil, bioclim) variables
    "Temp", "Temp_interval", "Temp_seasonality_interval", "Prec_seasonality_interval",
    "Prec", "Prec_interval",
...
### Costom order level ordering of ES

*Custom factor level ordering of ES variables for specific pedoclimatic, soil-related ecosystem services*
```{r}
# custom factor level ordering of ES variables
custom_ES_soil_order <- c(
    # Biodiversity ---------------------------------------------
    "Associated biodiversity",
    # Supporting ecosystem services ----------------------------
    "Soil formation",
...
# custom factor level ordering of ES variables for the selected soil-related ES
custom_selected_soil_ES_order <- c(
    #--# Biodiversity ---------------------------------------------
    "Associated biodiversity",
#--# Supporting ecosystem services ----------------------------
    "Soil formation",
...
es.systrait.sand.plot <- es.systrait.sand.data %>%
  ...
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  mutate(ES_ordered = factor(ES.GROUPS, levels = custom_ES_soil_order)) %>%
  arrange(ES_ordered) %>%
  na.omit() %>%
  
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_ES_soil_order),
    fill = sand_interval
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.sand.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
**Loading dataset**
```{r }
selected.soil.es.systrait.soc.data <-
  vroom::vroom("./DATASET.FROM.SCRIPT/selected.soil.es.systrait.soc.data.csv") 

es.systrait.soc.data <- 
  vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.soc.data.csv") 
```

**Matrix; selected soil-related ES (including biodiversity and categories data) with SOC intervals**
```{r PLOTTING - selected soil ES (including biodiversity and categories data) with SOC intervals, warning=FALSE, message=FALSE}
selected.soil.es.systrait.soc.plot <- selected.soil.es.systrait.soc.data %>%
  group_by(ES.GROUPS, soc_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  mutate(ES_ordered = factor(ES.GROUPS, levels = custom_selected_soil_ES_order)) %>%
  arrange(ES_ordered) %>%
  na.omit() %>%
  
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_selected_soil_ES_order),
    fill = `SAF System Trait type`
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
...
selected.soil.es.systrait.soc.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "selected.soil.es.systrait.pedoclim.soc.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.soc.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
**Loading dataset**
```{r }
selected.soil.es.systrait.bd.data <-
  vroom::vroom("./DATASET.FROM.SCRIPT/selected.soil.es.systrait.bd.data.csv") 

es.systrait.bd.data <- 
  vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.bd.data.csv") 
```

**Matrix; selected soil-related ES (including biodiversity and categories data) with bulk density intervals**
```{r PLOTTING - selected soil-related ES (including biodiversity and categories data) with bulk density intervals, warning=FALSE, message=FALSE}
selected.soil.es.systrait.bd.plot <- selected.soil.es.systrait.bd.data %>%
  group_by(ES.GROUPS, bd_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
...
#reported by peer-reviewed papers",
  ...
  facet_grid(. ~ factor(bd_interval,
                        levels = c("0.75 - 1 [kg dm^-3]", 
                                   "1 - 1.25 [kg dm^-3]", 
                                   "1.25 - 1.5 [kg dm^-3]", 
                                   "1.5 - 1.75 [kg dm^-3]", 
                                   "1.75 - 2 [kg dm^-3]")))

selected.soil.es.systrait.bd.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "selected.soil.es.systrait.bd.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
**Matrix; selected ES (including biodiversity and categories data) with bulk density intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with bulk density intervals, warning=FALSE, message=FALSE}
es.systrait.bd.plot <- es.systrait.bd.data %>%
  group_by(ES.GROUPS, bd_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.bd.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.ph.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.ph.data.csv") 
```

**Matrix; selected ES (including biodiversity and categories data) with bulk density intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with bulk density intervals, warning=FALSE, message=FALSE}
es.systrait.ph.plot <- es.systrait.ph.data %>%
  group_by(ES.GROUPS, ph_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
...
#reported by peer-reviewed papers",
  ...
  facet_grid(. ~ factor(ph_interval,
                        levels = c("pH 4 - 5", "pH 5 - 6", "pH 6 - 7", "pH 7 - 8", "pH 8 - 9")))
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.ph.plot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.isot.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.isot.data.csv") %>%
  mutate_if(is.character, as.factor)
```

Isothermality is defined as; Isothermality quantifies how large the day-to-night temperatures oscillate relative to the summer-to-winter (annual) oscillations. Isothermality is generally useful for biological entities (e.g. species and organisms). Units: Percent. Isothermality quantifies how large the day-to-night temperatures oscillate relative to the summer-to-winter (annual) oscillations. An isothermal value of 100 indicates the diurnal temperature range is equivalent to the annual temperature range, while anything less than 100 indicates a smaller level of temperature variability within an average month relative to the year. A species distribution may be influenced by larger or smaller temperature fluctuations within a month relative to the year and this predictor is useful for ascertaining such information.

**Matrix; selected ES (including biodiversity and categories data) with isothermality intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with isothermality intervals, warning=FALSE, message=FALSE}
es.systrait.isot.data.plot <- es.systrait.isot.data %>%
  group_by(ES.GROUPS, Isot_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  guides(size = guide_legend(title = ""), fill = FALSE) +
  labs(
    #title = "Matrix showing the frequency of associations of studied ecosystem services for different SAF systems across mean annual precipitation intervals 
#reported by peer-reviewed papers",
    #subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
   # caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks. Total number of independent studies reporting on ES = ", total_n_ES$value)),
    caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks. Total number of independent studies reporting on ES = ", total_n_ES$value)),
  ...
  facet_grid(. ~ factor(Isot_interval,
    levels = c("12 - 18 [%]", "18 - 24 [%]", "24 - 30 [%]", "30 - 36 [%]", "36 - 42 [%]", "42 - 48 [%]")
  ))

es.systrait.isot.data.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "es.systrait.pedoclim.isot.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
Temperature seasonality is defined as; the amount of temperature variation over a given year (or averaged years) based on the standard deviation (variation) of monthly temperature averages. Units: Temperature (degrees Celsius)

**Loading dataset**
```{r }

es.systrait.temp.seasonality.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.temp.seasonality.data.csv") 
```

**Matrix; selected ES (including biodiversity and categories data) with temperature seasonality intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with temperature intervals, warning=FALSE, message=FALSE}
es.systrait.temp.seasonality.plot <- es.systrait.temp.seasonality.data %>%
  group_by(ES.GROUPS, Temp_seasonality_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  arrange(ES_ordered) %>%
  na.omit() %>%
  filter(!Temp_seasonality_interval == "-10 - -5 [°C]") %>%
  
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_ES_soil_order),
    fill = Temp_seasonality_interval
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
    alpha = 0.8,
...
#reported by peer-reviewed papers",
  ...
  facet_grid(. ~ factor(Temp_seasonality_interval,
    levels = c("2.5 - 5 [°C]", 
               "5 - 7.5 [°C]", 
               "7.5 - 10 [°C]", 
               "10 - 12.5 [°C]", 
...
es.systrait.temp.seasonality.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "es.systrait.pedoclim.temp.seasonality.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.prec.seasonality.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.prec.seasonality.data.csv") %>%
  mutate_if(is.character, as.factor)
```

**Matrix; selected ES (including biodiversity and categories data) with precipitation seasonality intervals**
...
es.systrait.prec.seasonality.plot <- es.systrait.prec.seasonality.data %>%
  group_by(ES.GROUPS, Prec_seasonality_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  arrange(ES_ordered) %>%
  na.omit() %>%
  filter(!Prec_seasonality_interval == "100 - 120 [%]") %>%
  
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_ES_soil_order),
    fill = Prec_seasonality_interval
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
    alpha = 0.8,
...
#reported by peer-reviewed papers",
  ...
  facet_grid(. ~ factor(Prec_seasonality_interval,
    levels = c("0 - 20 [%]", "20 - 40 [%]", "40 - 60 [%]", "60 - 80 [%]", "80 - 100 [%]", "100 - 120 [%]")
  ))

es.systrait.prec.seasonality.plot
```
*Saving last plot*
```{r}
ggsave(
  filename = "es.systrait.pedoclim.prec.seasonality.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.temp.plot <- es.systrait.temp.data %>%
  group_by(ES.GROUPS, bio01_Annual_mean_temp_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  arrange(ES_ordered) %>%
  na.omit() %>%
  #filter(!bio01_Annual_mean_temp_interval == "-10 - -5 [°C]") %>%
  
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_ES_soil_order),
    fill = bio01_Annual_mean_temp_interval
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
    alpha = 0.8,
...
#reported by peer-reviewed papers",
  ...
  facet_grid(. ~ factor(bio01_Annual_mean_temp_interval,
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.temp.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.prec.plot <- es.systrait.prec.data %>%
  group_by(ES.GROUPS, bio12_Annual_prec_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  ggplot(aes(
    x = reorder(`SAF System Trait type`, -count),
    y = factor(ES_ordered, level = custom_ES_soil_order),
    fill = bio12_Annual_prec_interval
  )) +
  geom_jitter(aes(size = count),
    shape = 21, colour = "black",
    alpha = 0.8,
  ...
  facet_grid(. ~ factor(bio12_Annual_prec_interval,
...
```{r}
ggsave(
  filename = "es.systrait.pedoclim.prec.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.temp.bio10.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.temp.bio10.data.csv") 
```

**Matrix; selected ES (including biodiversity and categories data) with temperature extremes intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with temperature extremes intervals, warning=FALSE, message=FALSE}
es.systrait.temp.bio10.plot <- es.systrait.temp.bio10.data %>%
  group_by(ES.GROUPS, bio10_Mean_temp_warmest_quarter_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  facet_grid(. ~ factor(bio10_Mean_temp_warmest_quarter_interval,
    levels = c("7 - 10.5 [°C]", 
               "10.5 - 14 [°C]", 
               "14 - 17.5 [°C]",
               "17.5 - 21 [°C]",
...
es.systrait.temp.bio10.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "es.systrait.pedoclim.temp.bio10.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
...
es.systrait.prec.bio17.data <- vroom::vroom("./DATASET.FROM.SCRIPT/es.systrait.prec.bio17.data.csv") 
```

**Matrix; selected ES (including biodiversity and categories data) with precipitation/draught extremes intervals**
```{r PLOTTING - selected ES (including biodiversity and categories data) with precipitation/draught extremes intervals, warning=FALSE, message=FALSE}
es.systrait.prec.bio17.plot <- es.systrait.prec.bio17.data %>%
  group_by(ES.GROUPS, bio17_Prec_driest_quarter_interval, `SAF System Trait type`) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate_if(is.integer, as.double) %>%
  mutate_if(is.character, as.factor) %>%
  ...
  facet_grid(. ~ factor(bio17_Prec_driest_quarter_interval,
    levels = c("0 - 60 [mm]",
               "60 - 120 [mm]",
               "120 - 180 [mm]",
               "180 - 240 [mm]",
...
es.systrait.prec.bio17.plot
```

*Saving last plot*
```{r}
ggsave(
  filename = "es.systrait.pedoclim.prec.bio17.png",
  plot = last_plot(),
  path = "./FIGURES.OUTPUT.FROM.SCRIPT",
  width = 45, 
  height = 20, 
