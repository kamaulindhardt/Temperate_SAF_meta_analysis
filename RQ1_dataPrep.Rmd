...
```{r Merging the categories and ecosystem services datasets (renaming for easier plotting and saving the result), eval=FALSE}
...
# Saving the category.ecosystem.long dataset unto local disc
readr::write_csv(category.ecosystem.long, "./DATASET.FROM.SCRIPT/category.ecosystem.long.csv")
```

Loading category.ecosystem.long dataset from local disc
...
**Matrix - Associations of Tree - crop**

*Merging (joining) the crop and tree datasets based on ID.P and ID.S*
```{r Merging the crop and tree datasets}
joined.tree.crop <- 
  left_join(x = trees.data, y = crops.data, 
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
...
*Preparing crop - tree matrix data*
```{r Preparing crop - tree matrix data, warning=FALSE, message=FALSE}
# ---------------------------------------------------------------------------------------------------------------------------------------
# Preparing dataset on crop groups - tree dataset
  matrix.crop.groups.tree <- 
  joined.tree.crop %>%
  dplyr::select(CROP.OUTPUT.GROUP, TREE.GENUS.GROUP) %>%
  filter(TREE.GENUS.GROUP != "NA") %>%                      # excluding rows with 'NA' in either crop or tree data
...
*Plotting matrix - crop species - tree data*
...
matrix.crop.species.tree.all %>%
  ...
  scale_size_continuous(range = c(2, 10)) +
  scale_colour_discrete_qualitative(palette = "Dynamic") +
  labs(title = "Matrix showing the associations between tree genera and crop species reported in peer-reviewed papers",
       subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
       caption = paste0("n(independent studies) = ", n_independent_studies),
       x = "Tree genera",                       
       y = "Crop species") 
```
Assessing the frequency of associations occurrences for crop genera and tree genera
...
**Associations studied between crop groups and ecosystem services across SAF system traits**
Matrix of associations studied between crop groups and ecosystem services across SAF system traits

*Preparing ES, BD, CAT dataset*
```{r Preparing ES, BD, CAT}
# ------------------------------------------------------------------------------------------------------------------------------------
es.bd.cat.systrait.data <- 
  ...
  mutate_at(vars(-c(ID.P, ID.S, ST.TYPE)), tidyr::replace_na, 0) %>%   # <- Including NA's as 0 (FALSE)
  mutate_if(is.logical, as.numeric) %>%
...
# Saving the es.bd.cat.systrait.data.renamed dataset unto local disc
readr::write_csv(es.bd.cat.systrait.data.renamed, "./DATASET.FROM.SCRIPT/es.bd.cat.systrait.data.renamed.csv")
```

Loading es.bd.cat.systrait.data.renamed dataset from local disc and preparing and merging with crops
```{r Loading dataset with lat long coordinates}

es.bd.cat.systrait.data.renamed.load <- vroom::vroom("./DATASET.FROM.SCRIPT/es.bd.cat.systrait.data.renamed.csv") %>%
  dplyr::select(
...
joined.es.bd.cat.systrait.crop <- 
  left_join(x = es.bd.cat.systrait.data.renamed.load, y = crops.data, 
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
  select(-(ends_with(".x") | ends_with(".y"))) %>%
...
*Preparing a merged (joined) dataset of selected ecosystem services (including biodiversity and categories data) with crop based on ID.P and ID.S*
```{r Preparing merged dataset of selected ecosystem services (including biodiversity and categories data) with crop groups}
A_ES_ID <- 
  matrix.es.bd.cat.systrait.crop %>%
    dplyr::select(
    "ID.P",
...
# --------------------------------------------------------------------------------------------------------------------------
D_JOINED_A_B <- 
  left_join(x = A_ES_ID, y = B_SAFTT_ID,
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
  relocate(ID.P, ID.S,
...
# --------------------------------------------------------------------------------------------------------------------------
E_JOINED_D_C <- 
  left_join(x = D_JOINED_A_B, y = C_CROPS_ID,
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
  relocate(ID.P, ID.S,
...
*Plotting selected ecosystem services (including biodiversity and categories data) with crop groups*
...
E_JOINED_D_C.plot <- E_JOINED_D_C %>%
...
### Distribution of studies across different countries reporting on ES
...
D_JOINED_COUNTRIES_ES <- 
  left_join(x = A_COUNTRIES_ID, y = ES_ID,
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
  relocate(ID.P, ID.S,
           ES.GROUPS)
...
D_JOINED_COUNTRIES_CROPS_ES <- 
  left_join(x = D_JOINED_COUNTRIES_ES, y = B_CROPS_ID,
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S"))
# --------------------------------------------------------------------------------------------------------------------------
...
D_JOINED_COUNTRIES_CROPS_ES_SAFT <- 
  left_join(x = D_JOINED_COUNTRIES_CROPS_ES, y = C_SAFTT_ID,
            by = c("ID.P" = "ID.P",
                   "ID.S" = "ID.S")) %>%
  dplyr::select(-count) %>%
...
*Plotting selected ecosystem services (including biodiversity and categories data) with countries*
...
D_JOINED_COUNTRIES_CROPS_ES_SAFT %>%
  ...
  ggplot(aes(x = reorder(STUDY.LOC.COUNTRY, -count), 
             y = reorder(ES.GROUPS, -count),
  ...
  scale_size_continuous(range = c(1, 10)) +
  scale_fill_discrete_qualitative(palette = "Dynamic") +
  labs(title = "Matrix showing the frequency of associations of ecosystem services for different silvoarable systems across top ten most represented countries of publication reported by peer-reviewed papers",
       subtitle = paste("n (individual study sites) = ", n_individual_study_sites),
       caption = paste0(caption = paste0("* Line belts; shelterbelts, hedgerows and windbreaks. Total number of independent studies reporting on ES = ", total_n_ES$value)),
       x = "Countries",                       
       y = "Selected ecosystem services") +
...
management %>%
  ggplot(aes(x = "", y = count, fill = MANAGEMENT), alpha = 0.6) +
  geom_col(width = 1) +
  scale_fill_discrete_qualitative(palette = "Dynamic") +
  #geom_text(aes(label = count),
  #         position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y") +
  geom_label_repel(aes(y = management.pos$pos, label = paste0(pct* 100, "% - ", MANAGEMENT)),
                   size = 4.5, nudge_x = 1, show.legend = FALSE) +
 # guides(fill = guide_legend(title = "Group")) +
  scale_y_continuous(breaks = management.pos$pos, labels = management$MANAGEMENT) +
  theme_bw() +
...
ecosystem.data.renamed.DP <- 
  ...
  dplyr::select(ID.P,
                ...
                `Supporting ES`,
                `Cultural ES`) %>%
  pivot_longer(cols = ends_with("ES"),
               names_to = "ES",
               values_to = "count")
```

```{r}
study.approach.ecosystem <- left_join(x = study.approach, y = ecosystem.data.renamed.DP, 
            by = c("ID.P" = "ID.P"))  %>%
  relocate(ID.P) %>%
  mutate_if(is.character, as.factor) %>%
  select(!ID.P)
...
```
```{r}
ES.study.approach.PD.data.pie <- study.approach.ecosystem %>% 
  group_by(STUDY.FRAME) %>%
  rename("Approach" = STUDY.FRAME) %>%
  summarise(n = sum(count)) %>%
  ungroup() %>%
...
ES.study.approach.PD.data.pie$row <- 1

ES.study.approach.PD.data.pie %>%
ggplot(aes(x = row, y = n, fill = Approach)) +
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
    ...
    labs(title = "",
       caption = paste0("Total number of independent studies = ", n_independent_studies),
       x = "", 
       y = "Number of studies [log10 scale]") +
  coord_flip()
```
...
ecosystem.sum.data.PD <- 
  data.frame(value = apply(ecosystem.data.renamed, # the dataset ecosystem.data.renamed has been generated earlier in the script
                           MARGIN = 2, FUN = sum))
...
ecosystem.sum.data.PD.tier1.tier2 <- 
  ecosystem.sum.data.PD %>%
  dplyr::select(!key) %>%
  dplyr::filter(!ES.TIER2.GROUP %in% c("ID.P", # excluding ID.P and Tier 1 ES rows as that would not make sense in the Pie-Donut chart
                                      "Provisioning ES",
                                      "Regulating ES",
                                      "Supporting ES",
                                      "Cultural ES",
...
**Number of ES studies per paper**
